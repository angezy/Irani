...
const INTEGRATION_GROUPS = [
  ...
  {
    id: "zarinpal", title: "پرداخت زرین‌پال", target: "backend", description: "درخواست و تأیید پرداخت امن از طریق زرین‌پال.", requiredKeys: ["ZARINPAL_MERCHANT_ID", "ZARINPAL_CALLBACK_URL"],
    fields: [["ZARINPAL_MERCHANT_ID", "شناسه پذیرنده", "secret"], ["ZARINPAL_CALLBACK_URL", "نشانی بازگشت", "url"], ["ZARINPAL_SANDBOX", "حالت آزمایشی", "boolean"], ["ZARINPAL_AMOUNT_UNIT", "واحد مبلغ در درگاه", "text"]],
  },
  {
...
function placeholder(value) {
  return /^(?:replace-with|your-(?:sql|database|telegram|google)|owner@example\.com|example(?:@|\.com))/i.test(String(value || "").trim());
}
...
function validate(field, input) {
  ...
  if (field.key === "ZARINPAL_AMOUNT_UNIT" && !/^(?:RIAL|IRR|TOMAN|IRT)$/i.test(value)) return "must be RIAL or TOMAN";
  return null;
}
