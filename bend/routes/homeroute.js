...
function configuredStoreName() {
  return String(process.env.STORE_NAME || "فروشگاه ایرانی").trim().slice(0, 100) || "فروشگاه ایرانی";
}
...
function mapProductRow(row = {}, pricing = null) {
  ...
  return {
    ...
    currency: pricing?.currency || row.Currency || row.currency || "IRR",
...
function normalizeProductInput(body = {}) {
  ...
  return {
    ...
    currency: String(body.currency ?? body.Currency ?? "IRR").trim().toUpperCase().slice(0, 3) || "IRR",
...
const orderTrackingSteps = [
  { status: "Processing", title: "سفارش تأیید شد", description: "سفارش شما تأیید شده و برای ارسال آماده می‌شود." },
  { status: "Packed", title: "بسته‌بندی شد", description: "محصولات شما بررسی و برای ارسال بسته‌بندی شده‌اند." },
  { status: "Shipped", title: "تحویل به شرکت ارسال", description: "مرسوله شما از مرکز ارسال خارج شده است." },
  { status: "In Transit", title: "در مسیر ارسال", description: "مرسوله شما در شبکه ارسال در حال جابه‌جایی است." },
  { status: "Out for Delivery", title: "در حال تحویل", description: "مرسوله شما نزد تیم تحویل محلی است." },
  { status: "Delivered", title: "تحویل شد", description: "سفارش شما به مقصد رسیده است." },
];
...
function buildDefaultTracking(order) {
  ...
  const events = orderTrackingSteps.slice(0, eventCount).map((step, index) => ({
    ...
    location: index >= 2 ? (order.currentLocation || "شبکه توزیع") : `مرکز ارسال ${configuredStoreName()}`,
  ...
  return {
    ...
    estimatedDelivery,
    currentLocation: order.currentLocation || (progressIndex >= 2 ? "شبکه توزیع" : "مرکز ارسال"),
  };
}
...
async function loadOrderTracking(pool, userId, order) {
  ...
  try {
    ...
    const events = normalizeResult(result).map((row) => ({
      ...
      title: row.Title ?? row.title ?? row.Status ?? row.status ?? "به‌روزرسانی سفارش",
      description: row.Description ?? row.description ?? "وضعیت سفارش شما به‌روزرسانی شده است.",
...
function getPaymentProviderConfig() {
  ...
  return { ...config, provider: "zarinpal" };
}

function manualShippingOptions() {
  const standardCost = Math.max(0, Number(process.env.STANDARD_SHIPPING_AMOUNT) || 0);
  const expressCost = Math.max(0, Number(process.env.EXPRESS_SHIPPING_AMOUNT) || 0);
  return [
    { method: "standard", logisticName: "standard", label: "ارسال عادی", window: "۳ تا ۷ روز کاری", cost: standardCost, fromCountryCode: "" },
    { method: "express", logisticName: "express", label: "ارسال سریع", window: "۱ تا ۳ روز کاری", cost: expressCost, fromCountryCode: "" },
  ];
}

function selectManualShippingOption(options, requestedName) {
  const requested = String(requestedName || "").trim().toLowerCase();
  return options.find((option) => option.method === requested || option.logisticName === requested) || null;
}

function checkoutReturnUrl(status, authority, orderId = "") {
  const base = String(process.env.FRONTEND_URL || process.env.APP_BASE_URL || "").trim();
  if (!/^https?:\/\//i.test(base)) return null;
  const target = new URL("/checkout/return", base);
  target.searchParams.set("status", status);
  if (authority) target.searchParams.set("authority", String(authority));
  if (orderId) target.searchParams.set("orderId", String(orderId));
      ...
      .query(`
        ...
        BEGIN
          ...
          BEGIN
            ...
            VALUES
              (@CanonicalOrderId, @PreviousStatus, @OrderStatus, N'وضعیت ارسال فروشگاه');
          END;
        END;
...
function checkoutCurrency() {
  const value = String(process.env.STORE_CURRENCY || process.env.CHECKOUT_CURRENCY || "IRR").trim().toUpperCase();
  return /^[A-Z]{3}$/.test(value) ? value : "IRR";
}
...
function customerShippingServiceLabel(value) {
  return String(value || "").replace(/\s{2,}/g, " ").trim() || "روش ارسال";
}

function shippingDeliveryDays(windowValue) {
  const normalized = String(windowValue || "")
    .replace(/[۰-۹]/g, (digit) => "۰۱۲۳۴۵۶۷۸۹".indexOf(digit))
    .replace(/[٠-٩]/g, (digit) => "٠١٢٣٤٥٦٧٨٩".indexOf(digit));
  const days = normalized.match(/\d+/g)?.map(Number).filter(Number.isFinite) || [];
...
async function releaseDurableCheckout(pool, checkoutId, status = "Failed") {
  await pool.request()
    ...
    .input("PaymentStatus", sql.NVarChar(30), status === "Cancelled" ? "Cancelled" : "Failed")
    .query(`
      SET XACT_ABORT ON;
      BEGIN TRANSACTION;
      ...
      SET [checkout_status] = @Status,
          [payment_status] = CASE WHEN [payment_status] = N'Paid' THEN [payment_status] ELSE @PaymentStatus END,
          [payment_failed_at] = CASE WHEN @PaymentStatus = N'Failed' AND [payment_status] <> N'Paid' THEN COALESCE([payment_failed_at], SYSUTCDATETIME()) ELSE [payment_failed_at] END,
          [payment_cancelled_at] = CASE WHEN @PaymentStatus = N'Cancelled' AND [payment_status] <> N'Paid' THEN COALESCE([payment_cancelled_at], SYSUTCDATETIME()) ELSE [payment_cancelled_at] END,
...
async function activateDurableCheckout(pool, checkoutId, providerSessionId, gatewayAmount) {
  await pool.request()
    .input("CheckoutId", sql.UniqueIdentifier, checkoutId)
    .input("ProviderSessionId", sql.NVarChar(255), providerSessionId)
    .input("GatewayAmount", sql.BigInt, gatewayAmount)
    .query(`
      UPDATE [Commerce].[SecureCheckoutSessions]
      SET [provider_session_id] = @ProviderSessionId, [gateway_amount] = @GatewayAmount,
          [payment_requested_at] = COALESCE([payment_requested_at], SYSUTCDATETIME()),
          [checkout_status] = N'AwaitingPayment', [updated_at] = SYSUTCDATETIME()
...
async function loadDurableCheckoutByAuthority(pool, authority) {
  const result = await pool.request()
    .input("ProviderSessionId", sql.NVarChar(255), String(authority))
    .query("SELECT TOP 1 * FROM [Commerce].[SecureCheckoutSessions] WHERE [provider_session_id] = @ProviderSessionId");
  return normalizeResult(result)[0] || null;
}

async function markDurableCheckoutPaid(pool, checkoutId, { referenceId = null, gatewayAmount = null } = {}) {
  await pool.request()
    .input("CheckoutId", sql.UniqueIdentifier, checkoutId)
    .input("ReferenceId", sql.NVarChar(255), referenceId ? String(referenceId).slice(0, 255) : null)
    .input("GatewayAmount", sql.BigInt, Number.isSafeInteger(Number(gatewayAmount)) ? Number(gatewayAmount) : null)
    .query(`
    SET XACT_ABORT ON;
    BEGIN TRANSACTION;
    UPDATE [Commerce].[SecureCheckoutSessions]
    SET [checkout_status] = N'Paid', [payment_status] = N'Paid',
        [provider_payment_id] = COALESCE(NULLIF(@ReferenceId, N''), [provider_payment_id]),
        [gateway_amount] = COALESCE(@GatewayAmount, [gateway_amount]),
        [payment_verified_at] = COALESCE([payment_verified_at], SYSUTCDATETIME()),
  ...
  try {
    ...
    try {
      ...
      const mergeResult = await new sql.Request(transaction)
        ...
        .input("Currency", sql.Char(3), String(order.currency || "IRR").toUpperCase().slice(0, 3))
        .input("OrderStatus", sql.NVarChar(40), orderStatus)
        .input("FulfillmentStatus", sql.NVarChar(40), fulfillmentStatus)
...
router.get("/api/orders/track/:orderId", requireCheckoutIdentity, async (req, res) => {
  try {
    ...
    res.json({ order: { ...order, tracking: { ...tracking, sync: { provider: "manual", checkedAt: new Date().toISOString() } } } });
  } catch (err) {
...
router.post("/api/payment/create", requireCheckoutIdentity, async (req, res) => {
  ...
  const estimates = manualShippingOptions();
  const selectedShipping = selectManualShippingOption(estimates, requestedShippingMethod);
  if (!selectedShipping) {
  ...
  try {
    ...
    const paymentRequest = await requestZarinpalPayment({
      amount,
      description: `سفارش ${String(durableCheckout.id).slice(0, 36)}`,
      email: customerEmail,
      mobile: checkoutDetails.information?.phone,
    });
    await activateDurableCheckout(checkoutPool, durableCheckout.id, paymentRequest.authority, paymentRequest.gatewayAmount);

    const payment = {
      id: paymentRequest.authority,
      provider: "zarinpal",
      providerSessionId: paymentRequest.authority,
      ...
      shippingMethod,
      customerEmail,
      gatewayAmount: paymentRequest.gatewayAmount,
      checkoutUrl: paymentRequest.paymentUrl,
...
router.get("/api/payment/zarinpal/callback", async (req, res) => {
  const authority = String(req.query?.Authority || req.query?.authority || "").trim();
  const status = String(req.query?.Status || req.query?.status || "").trim();
  const redirectStatus = callbackSucceeded(status) ? "success" : "cancelled";
  const redirect = (result, orderId = "") => {
    const target = checkoutReturnUrl(result, authority, orderId);
    if (target) return res.redirect(303, target);
    return res.status(503).json({ error: "Payment callback URL is not configured" });
  };

  if (!authority) return redirect("failed");
  try {
    const pool = await getPool();
    const durable = await loadDurableCheckoutByAuthority(pool, authority);
    if (!durable) return redirect("failed");

    if (!callbackSucceeded(status)) {
      if (String(durable.payment_status || "").toLowerCase() !== "paid") {
        await releaseDurableCheckout(pool, durable.id, "Cancelled");
      }
      await recordCheckoutAttempt({ attemptId: authority, paymentId: authority, userId: durable.user_key, cartId: durable.user_key, customerEmail: durable.customer_email, status: "payment_cancelled", paymentError: "Customer cancelled the Zarinpal payment" });
      return redirect(redirectStatus, durable.order_id || "");
    }

    if (String(durable.payment_status || "").toLowerCase() === "paid") {
      return redirect("success", durable.order_id || "");
    }

    const verification = await verifyZarinpalPayment({ authority, amount: durable.total_amount });
    if (verification.gatewayAmount !== Number(durable.gateway_amount || 0)) {
      await releaseDurableCheckout(pool, durable.id, "Failed").catch(() => {});
      await recordCheckoutAttempt({ attemptId: authority, paymentId: authority, userId: durable.user_key, cartId: durable.user_key, customerEmail: durable.customer_email, status: "payment_failed", paymentError: "Verified gateway amount did not match the checkout amount" });
      return redirect("failed");
    }
    await markDurableCheckoutPaid(pool, durable.id, { referenceId: verification.refId, gatewayAmount: verification.gatewayAmount });
    await recordCheckoutAttempt({ attemptId: authority, paymentId: authority, userId: durable.user_key, cartId: durable.user_key, customerEmail: durable.customer_email, status: "payment_confirmed" });
    await recordSecurityEvent({ pool, eventType: "payment.zarinpal_verified", actor: null, resourceType: "checkout", resourceId: String(durable.id), metadata: { authority: authority.slice(0, 16), referenceId: verification.refId ? String(verification.refId).slice(0, 80) : null } });
    return redirect("success", durable.order_id || "");
  } catch (error) {
    console.error("Zarinpal callback processing failed:", error?.code || error?.message || "provider_error");
    return redirect("failed");
  }
});
...
router.post("/api/payment/confirm", requireCheckoutIdentity, async (req, res) => {
  ...
  try {
    ...
    if (!durable) return res.status(404).json({ error: "Durable checkout session was not found" });
    ...
    const storedGatewayAmount = Number(durable.gateway_amount || 0);
    if (!Number.isSafeInteger(storedGatewayAmount) || storedGatewayAmount < 10000) {
      return res.status(409).json({ error: "The checkout payment amount is invalid" });
    }

    let verification = null;
    if (String(durable.payment_status || "").toLowerCase() === "paid") {
      verification = { verified: true, alreadyVerified: true, refId: durable.provider_payment_id || null, gatewayAmount: storedGatewayAmount };
    } else {
      verification = await verifyZarinpalPayment({ authority: paymentId, amount: durable.total_amount });
      if (verification.gatewayAmount !== storedGatewayAmount) {
        await releaseDurableCheckout(checkoutPool, durable.id, "Failed").catch(() => {});
        return res.status(409).json({ error: "Payment amount does not match the checkout" });
      }
      await markDurableCheckoutPaid(checkoutPool, durable.id, {
        referenceId: verification.refId,
        gatewayAmount: verification.gatewayAmount,
      });
    }

    payment.status = "succeeded";
    payment.confirmedAt = new Date().toISOString();
    payment.referenceId = verification.refId || durable.provider_payment_id || null;
    payment.gatewayAmount = verification.gatewayAmount || storedGatewayAmount;
    await recordCheckoutAttempt({
      attemptId: payment.id,
      paymentId: payment.id,
      userId,
...
router.post("/api/orders/create", requireCheckoutIdentity, async (req, res) => {
  ...
  try {
    ...
    if (durable.order_id) {
      ...
      const fulfilledOrder = existingOrder;
      const fulfillment = { mode: "manual", submitted: false };
    ...
    if (durable.payment_status !== "Paid") return res.status(402).json({ error: "Payment confirmation is required before creating the order" });
  ...
  const shippingAddress = {
    ...
    fromCountryCode: String(persistedShipping.fromCountryCode || "").trim().toUpperCase(),
  };
...
router.post("/api/cart/shipping-estimate", requireCheckoutIdentity, async (req, res) => {
  ...
  try {
    ...
    const selected = selectManualShippingOption(estimates, requested) || estimates[0];
    res.json({ ok: true, scope: requestedProductId == null ? "cart" : "product", country, postalCode, estimates, selected });
  } catch (error) {
    console.error("/api/cart/shipping-estimate error", error?.message || error);
