...
test("two simultaneous stock=1 reservations produce exactly one winner", { skip }, async (t) => {
  ...
  for (const checkout of checkouts) {
    await pool.request().input("Id", sql.UniqueIdentifier, checkout).input("Key", sql.NVarChar(128), `fixture-${checkout}`).query(`
      ...
      VALUES (@Id,@Key,N'[]',N'IRR',10,0,0,10,N'standard',N'fixture@example.invalid',N'Reserving',N'Pending',DATEADD(MINUTE,30,SYSUTCDATETIME()))`);
  }
...
test("duplicate webhook IDs have one durable ledger row", { skip }, async (t) => {
  ...
  t.after(() => pool.request().input("Event", sql.NVarChar(255), eventId).query("DELETE FROM Integration.WebhookEvents WHERE provider=N'zarinpal' AND event_id=@Event"));
  const insert = () => pool.request().input("Event", sql.NVarChar(255), eventId).query(`
    INSERT INTO Integration.WebhookEvents (provider,event_id,event_type,payload_hash) VALUES (N'zarinpal',@Event,N'payment.callback','${"b".repeat(64)}')`);
  const results = await Promise.allSettled([insert(), insert()]);
  assert.equal(results.filter((result) => result.status === "fulfilled").length, 1);
  const count = (await pool.request().input("Event", sql.NVarChar(255), eventId).query("SELECT COUNT(*) count FROM Integration.WebhookEvents WHERE provider=N'zarinpal' AND event_id=@Event")).recordset[0].count;
