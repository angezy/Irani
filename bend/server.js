...
function validateProductionSecurityConfig() {
  ...
  if (String(process.env.PAYMENT_PROVIDER || "zarinpal").toLowerCase() === "zarinpal") {
    if (!String(process.env.ZARINPAL_MERCHANT_ID || "").trim()) throw new Error("ZARINPAL_MERCHANT_ID is required for Zarinpal payments");
    if (!String(process.env.ZARINPAL_CALLBACK_URL || "").trim()) throw new Error("ZARINPAL_CALLBACK_URL is required for Zarinpal payments");
  }
