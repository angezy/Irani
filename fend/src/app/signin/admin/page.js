...
export default function AdminSigninPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.success("خوش آمدید، مدیر", { duration: 700 });
      setTimeout(() => router.push("/dashboard"), 700);
    } catch (err) {
      setError(err.message || "ورود ناموفق بود");
      toast.error("ورود ناموفق بود", { description: err.message || "دوباره تلاش کنید." });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ padding: 28, minHeight: "100vh", background: "var(--color-background)", color: "var(--color-text-primary)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ padding: 28, background: "#ffffff", borderRadius: 16, width: "100%", maxWidth: 420, border: "1px solid var(--color-border)", boxShadow: "0 20px 60px rgba(43,43,43,0.08)" }}>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700 }}>ورود مدیر</h1>
        <p style={{ marginTop: 8, marginBottom: 20, color: 'var(--color-text-secondary)' }}>دسترسی به داشبورد فقط برای مدیران مجاز است.</p>

        <form onSubmit={handleSubmit} aria-describedby={error ? 'signin-error' : undefined}>
          <div style={{ display: 'grid', gap: 12 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 13 }}>ایمیل</span>
              <input
                type="email"
                value={email}
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 13 }}>رمز عبور</span>
              <input
                type="password"
                value={password}
