...
const DEFAULT_HEADER = {
  Name: process.env.NEXT_PUBLIC_STORE_NAME || "فروشگاه ایرانی",
  Home: "خانه",
  Blog: "مجله",
  Shop: "فروشگاه",
  AboutUs: "درباره ما",
...
const categoryLinks = [
  { label: "تمرین", href: "/category/training" },
  { label: "قدرتی", href: "/category/strength" },
  { label: "ریکاوری", href: "/category/recovery" },
];

const aboutLinks = [
  { label: `درباره ${process.env.NEXT_PUBLIC_STORE_NAME || "فروشگاه ایرانی"}`, href: "/aboutus" },
  { label: "چطور کار می‌کند؟", href: "/how-it-works" },
  { label: `چرا ${process.env.NEXT_PUBLIC_STORE_NAME || "فروشگاه ایرانی"}؟`, href: "/why-weluxo" },
...
export default function Header({ initialHeader = null, initialChrome = null, disableNav = false }) {
  ...
  const brandName = siteSettings.siteName || displayHeader.Name || process.env.NEXT_PUBLIC_STORE_NAME || "فروشگاه ایرانی";
  const brandLogo = siteSettings.siteLogoUrl || displayHeader.LogoUrl || "";
  const brandTagline = siteSettings.siteTagline || "با اطمینان خرید کنید";
  ...
  const loadCart = async () => {
    ...
    try {
      ...
      setCartError(error.message || "بارگذاری سبد خرید ممکن نیست.");
    } finally {
  ...
  const handleRemoveCartItem = async (productId) => {
    ...
    try {
      ...
      setCartError(error.message || "حذف محصول از سبد خرید ممکن نیست.");
    } finally {
  ...
  const handleCartQuantityChange = async (item, quantity) => {
    ...
    try {
      ...
      setCartError(error.message || "به‌روزرسانی تعداد محصول ممکن نیست.");
    } finally {
      setCartPendingItemId(null);
    }
  };
    ...
    <Box component="header" sx={{ position: "sticky", top: 0, zIndex: (theme) => theme.zIndex.appBar, bgcolor: "#ffffff", color: "var(--color-text-primary)", borderBottom: "1px solid var(--color-border)" }}>
      ...
      <AppBar position="static" elevation={0} color="transparent" sx={{ color: "inherit", bgcolor: "white" }}>
        <Toolbar sx={{ minHeight: { xs: 68, md: 82 }, px: { xs: 1.5, sm: 3, md: 5 }, gap: { xs: 1, md: 3 } }}>
          <IconButton onClick={() => setMobileOpen(true)} aria-label="باز کردن ناوبری" sx={{ display: { md: "none" }, color: "var(--color-text-primary)" }}>
            <MenuIcon />
          </IconButton>
          ...
          <Box component="nav" aria-label="ناوبری اصلی" sx={{ display: { xs: "none", md: "flex" }, alignItems: "center", justifyContent: "center", gap: 0.5, borderTop: "1px solid var(--color-border)", px: 3, minHeight: 49 }}>
      ...
      <Menu anchorEl={shopAnchor} open={shopOpen} onClose={() => setShopAnchor(null)} MenuListProps={{ "aria-label": "دسته‌بندی‌های فروشگاه", onMouseEnter: handleShopMenuEnter, onMouseLeave: handleShopMenuLeave }} slotProps={{ root: { sx: { pointerEvents: "none" } }, paper: { sx: { pointerEvents: "auto" } } }}>
      ...
      <Menu anchorEl={accountAnchor} open={accountOpen} onClose={() => setAccountAnchor(null)} MenuListProps={{ "aria-label": "منوی حساب کاربری" }}>
        ...
        <MenuItem component={Link} href="/account/saved" onClick={handleNav("/account/saved")} sx={headerMenuLinkSx}>محصولات ذخیره‌شده</MenuItem>
      ...
      <Menu
        ...
        autoFocus={false}
        MenuListProps={{ "aria-label": "منوی درباره ما", onMouseEnter: handleAboutMenuEnter, onMouseLeave: handleAboutMenuLeave }}
                ...
                return (
                  <ListItem key={item.productId} disableGutters sx={{ py: 0.75, alignItems: "center" }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={0.25} flexShrink={0}>
                      ...
                      <Typography component="span" aria-label={`${item.title || "محصول"}، تعداد: ${quantity}`} sx={{ minWidth: 18, textAlign: "center", fontSize: 13, fontWeight: 800 }}>
                        {quantity}
                      </Typography>
                      <IconButton size="small" aria-label={`افزایش تعداد ${item.title || "محصول"}`} onClick={() => handleCartQuantityChange(item, quantity + 1)} disabled={itemPending}>
                        <Add fontSize="small" />
                      </IconButton>
                      <IconButton size="small" aria-label={`حذف ${item.title || "محصول"} از سبد خرید`} onClick={() => handleRemoveCartItem(item.productId)} disabled={itemPending} sx={{ color: "var(--color-text-secondary)", "&:hover": { color: "error.main", bgcolor: "rgba(211, 47, 47, 0.08)" } }}>
