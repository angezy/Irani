    ...
    <Box component="section" aria-labelledby="contact-support-title" sx={{ bgcolor: "#ffffff", color: "var(--color-text-primary)", border: "1px solid var(--color-border)", boxShadow: "0 14px 40px rgba(43,43,43,0.07)", borderRadius: { xs: 3, md: 5 }, p: { xs: 3, md: 6 }, mt: { xs: 6, md: 9 }, mb: { xs: 2, md: 4 } }}>
      <Grid container spacing={{ xs: 4, md: 6 }}>
        <Grid
          ...
          <Typography variant="overline" sx={{ color: "var(--color-accent)", letterSpacing: "0.14em", fontWeight: 800, display: "block", mb: 1 }}>{content.eyebrow || "پشتیبانی اختصاصی"}</Typography>
          <Typography id="contact-support-title" component="h2" sx={{ fontWeight: 850, letterSpacing: "-0.05em", fontSize: { xs: "2.7rem", md: "4.2rem" }, lineHeight: 0.98, mb: 2 }}>{content.title || `ارتباط با ${siteSettings.siteName}`}</Typography>
          ...
          <Stack spacing={1.5}>
            ...
            <Box sx={{ mt: 1.5, p: 2, border: "1px solid color-mix(in srgb, var(--color-primary) 22%, transparent)", bgcolor: "var(--color-primary-soft)", borderRadius: 2.5 }}>
              <Typography sx={{ fontWeight: 800, mb: 0.5 }}>درخواست‌های پشتیبانی من</Typography>
              <Typography sx={{ color: "var(--color-text-secondary)", fontSize: 14, lineHeight: 1.6, mb: 1.5 }}>درخواست‌های باز، پاسخ‌ها و سوابق پشتیبانی را در حساب کاربری ببینید.</Typography>
              <Button component={Link} href="/account/support" variant="contained" startIcon={<ContactMailOutlinedIcon />} sx={{ bgcolor: "var(--color-primary)", color: "#ffffff", borderRadius: 999, textTransform: "none", fontWeight: 800, "&:hover": { bgcolor: "var(--color-primary-dark)" } }}>مشاهده درخواست‌ها</Button>
            </Box>
          </Stack>
        </Grid>
      </Grid>
