...
export default function ConditionalShell({ children, initialChrome = null }) {
  const pathname = usePathname()
  const hideShell = pathname?.startsWith('/dashboard') || pathname?.startsWith('/admin')

  return (
    <>
      {!hideShell && <Header initialChrome={initialChrome} />}
      {children}
      {!hideShell && <Footer initialChrome={initialChrome} />}
