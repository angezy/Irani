...
function titleFor(product = {}) {
  return product.title || product.name || product.Name || "محصول بدون عنوان";
}

function priceFor(value) {
  return value > 0 ? formatMoney(value) : "مشاهده محصول";
}
...
export default function ResumeShoppingSection({ products = [] }) {
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, md: 4 }, py: 4, color: "#111827" }}>
      <Box sx={{ mb: 2 }}>
        <Typography variant="h5" sx={{ fontWeight: 900 }}>ادامه از جایی که ماندید</Typography>
        <Typography sx={{ color: "#526074", mt: 0.4, fontSize: 14 }}>
          مرور محصولاتی را که اخیراً دیده‌اید ادامه دهید.
        </Typography>
        <Typography sx={{ mt: 0.4, color: "#526074", fontSize: 14 }}>
          برای دیدن پیشنهادهای شخصی‌تر، <Link href="/signin" style={{ color: "var(--color-primary)" }}>وارد شوید</Link>.
        </Typography>
      </Box>

      {recentProducts.length > 0 && (
        <Stack direction="row" spacing={2} sx={{ overflowX: "auto", pb: 1, mb: 4 }}>
          {recentProducts.slice(0, 4).map((product) => <ProductTile key={`recent-${product.key || product.slug}`} product={product} label="اخیراً مشاهده‌شده" />)}
        </Stack>
      )}
