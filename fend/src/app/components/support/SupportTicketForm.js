...
const categories = [["Order", "سفارش"], ["Shipping", "ارسال"], ["Payment", "پرداخت"], ["Refund", "بازپرداخت"], ["Returns", "مرجوعی"], ["Product Question", "پرسش محصول"], ["Warranty", "گارانتی"], ["Technical Issue", "مشکل فنی"], ["Account", "حساب کاربری"], ["Partnership", "همکاری"]];
...
export default function SupportTicketForm() {
  ...
  const submit = async (event) => {
    ...
    setError("");
    setSuccess("");
    if (!textFromHtml(form.contentHtml)) return setError("لطفاً پیش از ارسال، پیام خود را بنویسید.");
    setSaving(true);
    try {
  ...
  return (
    <Box component="main" sx={{ bgcolor: "var(--color-background)", minHeight: "100vh", py: { xs: 3, md: 6 }, color: "var(--color-text-primary)" }}>
      <Container maxWidth="md">
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={2} sx={{ mb: 3 }}><Box><Typography variant="overline" sx={{ color: "var(--color-primary)", fontWeight: 800, letterSpacing: "0.14em" }}>درخواست پشتیبانی</Typography><Typography component="h1" sx={{ fontWeight: 850, letterSpacing: "-0.05em", fontSize: { xs: "2.5rem", md: "4rem" }, lineHeight: 1 }}>ایجاد درخواست پشتیبانی</Typography><Typography sx={{ color: "var(--color-text-secondary)", mt: 2, lineHeight: 1.7 }}>موضوع را توضیح دهید. اگر درخواست شما درباره خرید است، شماره سفارش را هم بنویسید.</Typography></Box><Button component={Link} href="/account/support" sx={{ color: "var(--color-primary)", textTransform: "none", fontWeight: 800 }}>بازگشت به پشتیبانی</Button></Stack>
          ...
          <Grid container spacing={2}>
            <Grid
              size={{
                xs: 12,
                sm: 6
              }}><TextField required fullWidth label="نام مشتری" value={form.customerName} onChange={(event) => update("customerName", event.target.value)} /></Grid>
            <Grid
              size={{
                xs: 12,
                sm: 6
              }}><TextField required fullWidth type="email" label="ایمیل" value={form.email} onChange={(event) => update("email", event.target.value)} /></Grid>
            <Grid
              size={{
                xs: 12,
                sm: 6
              }}><TextField fullWidth label="شماره سفارش" value={form.orderNumber} onChange={(event) => update("orderNumber", event.target.value)} /></Grid>
            <Grid
              size={{
                xs: 12,
                sm: 3
              }}><TextField select fullWidth label="دسته‌بندی" value={form.category} onChange={(event) => update("category", event.target.value)}>{categories.map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</TextField></Grid>
            <Grid
              size={{
                xs: 12,
                sm: 3
