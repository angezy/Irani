...
const STATUS_LABELS = { New: "جدید", Open: "باز", "In Progress": "در حال بررسی", "Waiting for Customer": "در انتظار مشتری", Resolved: "حل‌شده", Closed: "بسته‌شده" };
const typeLabel = (value) => STATUS_LABELS[String(value || "")] || value || "نامشخص";
...
export default function SupportTicketConversation({ ticketId }) {
  ...
  const load = () => fetch(`/api/support/tickets/${ticketId}`, { credentials: "include" })
    .then((response) => response.ok ? response.json() : response.json().then((data) => Promise.reject(new Error(data.error || "بارگذاری درخواست ممکن نیست"))))
  ...
  const send = async (event) => {
    ...
    if (!textFromHtml(message)) return setError("لطفاً ابتدا پیام خود را بنویسید.");
    setSending(true);
    setError("");
    try {
  ...
  if (error && !ticket) return <Container sx={{ py: 8 }}><Alert severity="error">{error}</Alert><Button component={Link} href="/signin" sx={{ mt: 2 }}>ورود</Button></Container>;
        ...
        <Paper elevation={0} sx={{ border: "1px solid var(--color-border)", borderRadius: 3, overflow: "hidden" }}>
          <Box sx={{ bgcolor: "var(--color-primary-dark)", color: "white", p: { xs: 2.5, md: 4 } }}>
            <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={2}>
              ...
              <Chip label={typeLabel(ticket.status)} sx={{ alignSelf: "flex-start", bgcolor: "var(--color-primary-soft)", color: "var(--color-primary-dark)", fontWeight: 800 }} />
            </Stack>
          </Box>
          ...
          <Box component="form" onSubmit={send} sx={{ p: { xs: 2, md: 4 }, borderTop: "1px solid var(--color-border)" }}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", sm: "center" }} gap={1.5} sx={{ mt: 2 }}>
              ...
              <Button type="submit" disabled={sending} variant="contained" startIcon={<SendIcon />} sx={{ borderRadius: 999, bgcolor: "var(--color-primary)", textTransform: "none", fontWeight: 800 }}>{sending ? "در حال ارسال…" : "ارسال پاسخ"}</Button>
            </Stack>
