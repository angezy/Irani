      ...
      <Container maxWidth="lg" sx={{ px: { xs: 2, md: 4 }, py: { xs: 2, md: 4 } }}>
        <Box component="header" sx={{ position: "relative", bgcolor: "#ffffff", color: "var(--color-text-primary)", border: "1px solid var(--color-border)", borderRadius: { xs: 3, md: 5 }, overflow: "hidden", p: { xs: 3, md: 7 }, mb: { xs: 6, md: 9 } }}>
          ...
          <Box sx={{ position: "relative", zIndex: 1, maxWidth: 850 }}>
            ...
            />
            {normalizedQuery && <Typography sx={{ color: "var(--color-accent)", fontSize: 13, mt: 1.5 }}>{visibleCategories.reduce((count, category) => count + category.articles.length, 0) + visibleFaq.length} نتیجه برای «{query}»</Typography>}
          </Box>
        </Box>
            ...
            {visibleCategories.map((category, index) => (
              <Accordion key={`${category.title}-${index}`} expanded={expandedCategory === index} onChange={() => setExpandedCategory(expandedCategory === index ? -1 : index)} disableGutters elevation={0} sx={{ bgcolor: "transparent", borderTop: "1px solid var(--color-border)", "&:last-child": { borderBottom: "1px solid var(--color-border)" }, "&::before": { display: "none" } }}>
                ...
                <AccordionDetails id={`category-panel-${index}`} sx={{ px: 0, pt: 0, pb: 3 }}>
                  ...
                  ) : <Typography sx={{ color: "var(--color-text-secondary)" }}>مقاله‌ای مطابق جست‌وجو در این دسته پیدا نشد.</Typography>}
                </AccordionDetails>
              </Accordion>
            ))}
            {normalizedQuery && visibleCategories.length === 0 && <Typography sx={{ color: "var(--color-text-secondary)", py: 3 }}>مقاله راهنمایی مطابق جست‌وجو پیدا نشد. عبارت دیگری را امتحان کنید.</Typography>}
          </Box>
        </Box>
        ...
        <Box component="section" aria-labelledby="help-faq-title" sx={{ maxWidth: 920, mx: "auto", position: "relative", mb: { xs: 8, md: 12 } }}>
          ...
          {normalizedQuery && visibleFaq.length === 0 && <Typography sx={{ color: "var(--color-text-secondary)", py: 3 }}>پاسخی مطابق جست‌وجو پیدا نشد.</Typography>}
        </Box>
      </Container>
    </Box>
  );
