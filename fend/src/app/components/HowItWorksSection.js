...
function Label({ children, light = false }) {
  return (
    <Typography
      variant="overline"
      sx={{
        color: light ? "var(--color-accent)" : "var(--color-primary)",
  ...
  return String(text || "")
    ...
    .map((paragraph, index) => (
      <Typography
        key={index}
        sx={{
          color: light ? "var(--color-text-secondary)" : "var(--color-text-secondary)",
          lineHeight: 1.8,
          mb: 2,
        }}
      >
    ...
    <Button
      ...
      sx={{
        ...
        ...(light && variant === "contained"
          ? { bgcolor: "var(--color-accent)", color: "var(--color-text-primary)", "&:hover": { bgcolor: "var(--color-accent-dark)" } }
          : {}),
      }}
    >
      {children}
  ...
  return (
    <Box
      sx={{
        ...
        bgcolor: index % 2 ? "var(--color-accent-soft)" : "var(--color-primary-soft)",
    ...
    >
      <Box sx={{ position: "absolute", width: 240, height: 240, borderRadius: "50%", bgcolor: "rgba(242,140,40,0.12)", top: -90, right: -70 }} />
      <Box sx={{ position: "relative", zIndex: 1 }}>
        <Box sx={{ width: 54, height: 54, display: "grid", placeItems: "center", borderRadius: 2.5, bgcolor: "var(--color-primary)", color: "#ffffff", mb: 4 }}>
          <Icon />
        </Box>
        {step.image ? (
          ...
          <Stack spacing={1.25} sx={{ mb: 2 }}>
            {flow.map((item, flowIndex) => (
              <Box key={`${item}-${flowIndex}`} sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
                <Box sx={{ width: 28, height: 28, borderRadius: "50%", display: "grid", placeItems: "center", bgcolor: "var(--color-primary)", color: "#ffffff", fontSize: 12, fontWeight: 800, flexShrink: 0 }}>
  ...
  return (
    <Box component="main" sx={{ bgcolor: "var(--color-background)", color: "var(--color-text-primary)", minHeight: "100vh", fontFamily: "var(--site-font-family, 'Space Grotesk','Segoe UI',sans-serif)" }}>
      <Container maxWidth="lg" sx={{ px: { xs: 2, md: 4 }, py: { xs: 2, md: 4 } }}>
        <Box component="section" aria-labelledby="how-it-works-title" sx={{ position: "relative", bgcolor: "#ffffff", color: "var(--color-text-primary)", border: "1px solid var(--color-border)", borderRadius: { xs: 3, md: 5 }, overflow: "hidden", display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.04fr 0.96fr" }, minHeight: { md: 550 } }}>
          ...
          <Box sx={{ p: { xs: 3, md: 7 }, display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", zIndex: 1 }}>
            ...
            <Typography sx={{ color: "var(--color-text-secondary)", lineHeight: 1.78, maxWidth: 560, mt: 3, fontSize: { xs: "1rem", md: "1.1rem" } }}>
              {hero.copy}
            </Typography>
        ...
        <Box component="section" aria-labelledby="details-title" sx={{ position: "relative" }}>
          <Typography id="details-title" component="h2" sx={{ position: "absolute", width: 1, height: 1, overflow: "hidden", clip: "rect(0 0 0 0)" }}>فرایند خرید از فروشگاه چگونه انجام می‌شود</Typography>
              ...
              return (
                <Box component="article" id={`how-step-${step.id || index + 1}`} key={step.id || index} sx={{ position: "relative", display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: { xs: 3, md: 7 }, alignItems: "center" }}>
                  ...
                  <Box sx={{ order: { xs: 1, md: isReverse ? 2 : 1 } }}>
                    <Label>{step.eyebrow || `مرحله ${step.number}`}</Label>
