...
export default function CustomerReviewsSection() {
  ...
  async function submitReview(event) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "ثبت نظر ممکن نیست");

      setForm((current) => ({ ...current, title: "", text: "", rating: 5 }));
      setMessage(data.message || "سپاسگزاریم. نظر شما پس از بررسی منتشر خواهد شد.");
    } catch (submitError) {
      setError(submitError.message || "ثبت نظر ممکن نیست");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <Box component="form" onSubmit={submitReview}>
          <Grid container spacing={2}>
            <Grid
              ...
              <TextField label="نام شما" value={form.name} onChange={updateField("name")} required fullWidth />
            </Grid>
            <Grid
              size={{
                xs: 12,
              ...
              <TextField label="ایمیل (اختیاری)" type="email" value={form.email} onChange={updateField("email")} fullWidth />
            </Grid>
            <Grid
              size={{
                xs: 12,
              ...
              <TextField label="عنوان نظر" value={form.title} onChange={updateField("title")} fullWidth />
            </Grid>
            <Grid
              size={{
                xs: 12,
              ...
              <Stack direction="row" spacing={1} alignItems="center" sx={{ height: "100%", minHeight: 56 }}>
                <Typography sx={{ color: "var(--color-text-secondary)", whiteSpace: "nowrap" }}>امتیاز شما</Typography>
                <Rating value={Number(form.rating)} onChange={(_, value) => setForm((current) => ({ ...current, rating: value || 5 }))} />
              </Stack>
            </Grid>
            <Grid size={12}>
              <TextField
                label="درباره تجربه خود بنویسید"
                value={form.text}
