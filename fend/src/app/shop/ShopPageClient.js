...
import defaultShopContent from "../../../data/shop.json";

const DEFAULT_SHOP_CONTENT = defaultShopContent;
...
function ShopProductCard({ product, title, description, price, image, trending = false, onAdd, adding = false }) {
  return (
    ...
    >
      <Box component={Link} href={productHref(product, title)} sx={{ display: "flex", flex: 1, flexDirection: "column", color: "inherit", textDecoration: "none", "&:focus-visible": { outline: "3px solid var(--color-primary-light)", outlineOffset: -3 } }} aria-label={`مشاهده ${title}`}>
        ...
        <CardContent sx={{ flexGrow: 1, display: "flex", flexDirection: "column", minHeight: trending ? 190 : 210 }}>
          {trending && <Chip label="پرفروش" size="small" sx={{ alignSelf: "flex-start", mb: 1.5, backgroundColor: "var(--color-accent-soft)", color: "var(--color-accent-dark)", fontWeight: 700 }} />}
...
export default function ShopPageClient({ initialContent = null, editable = false, onEdit = {} }) {
  ...
  async function handleAddProduct(product) {
    const title = product.name || product.Name || product.title || "بدون عنوان";
    ...
    try {
      ...
      toast.success("به سبد خرید افزوده شد", {
        description: `${title} اکنون در سبد خرید شماست.`,
      ...
      if (err.message === "unauthorized") {
        setSessionUser(null);
        toast.info("نشست سبد خرید منقضی شده است", { description: "لطفاً دوباره کالا را اضافه کنید." });
      } else {
        toast.error("افزودن کالا انجام نشد", { description: err.message || "لطفاً دوباره تلاش کنید." });
      }
    } finally {
  ...
  return (
    <Box sx={{ backgroundColor: "var(--color-background)", minHeight: "100vh", color: "var(--color-text-primary)" }}>
      ...
      <Container id="shop-catalog" maxWidth="lg" sx={{ py: 6, position: "relative" }}>
        {editButton("catalog", "فهرست محصولات")}
        {trendingProducts.length > 0 && (
          <Box component="section" aria-labelledby="trending-collection-title" sx={{ mb: 7 }}>
            ...
            <Typography variant="body1" sx={{ color: "var(--color-text-secondary)", mb: 3 }}>
              محصولاتی که این روزها بیشتر مورد توجه قرار گرفته‌اند.
            </Typography>
            <Box sx={{ display: "flex", justifyContent: { xs: "flex-start", lg: "center" }, gap: 2, overflowX: "auto", pb: 2, scrollbarWidth: "thin" }}>
              {trendingProducts.map((product) => {
                const title = product.name || product.Name || product.title || "بدون عنوان";
                const description = product.Description || product.description || "توضیحی برای این محصول ثبت نشده است.";
                const price = typeof product.price === "number" ? product.price : Number(product.Price) || 0;
                const firstImage = Array.isArray(product.images) && product.images.length > 0 ? product.images[0] : null;
                const image = firstImage || product.Img || product.IMG || product.img || product.image || product.imageUrl ||
                  "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=800&q=80";
