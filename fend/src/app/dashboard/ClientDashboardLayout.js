...
function DashboardAccessDenied() {
  return <main style={{ minHeight: '100%', padding: '64px 32px', background: '#f8fafc' }}><section style={{ maxWidth: 640, margin: '0 auto', background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: 32 }}><p style={{ color: '#64748b', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase' }}>۴۰۳ · دسترسی غیرمجاز</p><h1 style={{ margin: '8px 0 12px', color: '#0f172a' }}>شما به این بخش از داشبورد دسترسی ندارید.</h1><p style={{ color: '#475569' }}>از مدیر فروشگاه بخواهید مجوز لازم را برای شما فعال کند.</p></section></main>
}
...
export default function ClientDashboardLayout({ children, user }) {
  ...
  const siteName = useSiteSettings().siteName || 'فروشگاه ایرانی'
