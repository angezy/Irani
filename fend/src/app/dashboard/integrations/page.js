  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1400, mx: "auto", minHeight: "100%" }}>
      ...
      <Alert severity="warning" icon={<WarningAmberOutlinedIcon />} sx={{ mb: 3, borderRadius: 2 }}>
        اطلاعات محرمانه مخفی می‌مانند و هرگز به‌صورت ساده بازگردانده نمی‌شوند. کلیدهای خصوصی را در متغیرهای <code>NEXT_PUBLIC_*</code> قرار ندهید و فایل واقعی <code>.env</code> را ثبت نکنید. {note}
      </Alert>
              ...
              <Card sx={{ height: "100%", borderRadius: 3, border: "1px solid #e2e8f0", boxShadow: "0 8px 25px rgba(15,23,42,0.05)" }}>
                <CardContent sx={{ p: { xs: 2, md: 2.5 } }}>
                  ...
                  <Stack spacing={0.8}>{group.fields.map((field) => <Stack key={field.key} direction="row" justifyContent="space-between" alignItems="center" spacing={2} sx={{ minHeight: 31 }}>
                    ...
                    <Typography sx={{ flexShrink: 0, maxWidth: "48%", color: field.configured ? "#475569" : "#cbd5e1", fontSize: 12, fontFamily: field.type === "secret" || field.value ? "monospace" : "inherit", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{field.placeholderValue ? "مقدار نمونه" : field.value || "تنظیم نشده است"}</Typography>
                  </Stack>)}</Stack>
