...
export default function CheckoutInformationPage() {
  ...
  function handleSubmit(event) {
    ...
    if (!form.email.trim() || !form.phone.trim()) {
      setError("برای ادامه، ایمیل و شماره تماس را وارد کنید.");
      toast.warning("اطلاعات لازم است", { description: "برای ادامه، ایمیل و شماره تماس را وارد کنید." });
      return;
    }
    updateCheckoutState({
      information: {
    ...
    <CheckoutLayout currentStep="information" items={items} subtotal={subtotal} discount={discount} couponCode={couponCode} shippingMethod={readCheckoutState().shipping}>
      ...
      <Card sx={{ bgcolor: "#ffffff", color: "var(--color-text-primary)", border: "1px solid var(--color-border)" }}>
        <CardContent sx={{ p: { xs: 3, md: 4 } }}>
          <Typography variant="h5" sx={{ fontWeight: 900, mb: 1 }}>اطلاعات مشتری</Typography>
          <Typography sx={{ color: "var(--color-text-secondary)", mb: 3 }}>از این اطلاعات برای اطلاع‌رسانی سفارش و هماهنگی ارسال استفاده می‌کنیم.</Typography>
          <Box component="form" onSubmit={handleSubmit}>
            <Stack spacing={2}>
              <TextField label="نشانی ایمیل" type="email" value={form.email} onChange={update("email")} required fullWidth autoComplete="email" sx={fieldSx} />
              <TextField label="شماره تماس" type="tel" value={form.phone} onChange={update("phone")} required fullWidth autoComplete="tel" sx={fieldSx} />
              <FormControlLabel control={<Checkbox checked={Boolean(form.subscribe)} onChange={(event) => setForm((current) => ({ ...current, subscribe: event.target.checked }))} />} label="دریافت پیشنهادهای Weluxo" />
              {user?.guest && (
                <Alert severity="info">خرید مهمان فعال است. برای دسترسی به سوابق سفارش و اطلاعات ذخیره‌شده، <Link href="/signin" style={{ color: "inherit", fontWeight: 700 }}>وارد شوید</Link>.</Alert>
              )}
              <Button type="submit" variant="contained" size="large" sx={{ borderRadius: 999, py: 1.3, fontWeight: 800 }}>ادامه به اطلاعات ارسال</Button>
            </Stack>
          </Box>
        </CardContent>
      </Card>
