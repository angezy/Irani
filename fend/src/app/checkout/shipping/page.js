...
export default function CheckoutShippingPage() {
  ...
  async function loadShippingOptions() {
    if (!String(form.country || "").trim() || !String(form.postalCode || "").trim()) {
      throw new Error("کشور را انتخاب و کد پستی را وارد کنید.");
    }
    ...
    try {
      ...
      const selected = options.find((option) => option.logisticName === form.logisticName || option.method === form.method)
        || result.selected
        || options[0];
      if (!selected) throw new Error("برای این سبد و نشانی، روش ارسالی موجود نیست.");
  ...
  async function handleLoadShippingOptions() {
    try {
      ...
      setError(quoteError.message || "بارگذاری روش‌های ارسال ناموفق بود.");
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    if (required.some((field) => !String(form[field] || "").trim())) {
      setError("همه فیلدهای الزامی ارسال را کامل کنید.");
      toast.warning("اطلاعات ارسال لازم است", { description: "همه فیلدهای الزامی ارسال را کامل کنید." });
      return;
    }
    ...
    if (!normalizeShippingSelection(form) || !shippingOptions.some((option) => (option.logisticName || option.method) === (form.logisticName || form.method))) {
      try {
        ...
        setError(quoteError.message || "پیش از ادامه، یک روش ارسال موجود انتخاب کنید.");
        toast.warning("روش ارسال لازم است", { description: quoteError.message || "پیش از ادامه، یک روش ارسال موجود انتخاب کنید." });
        return;
      }
    }
    ...
    if (user && !user.guest) {
      try {
        ...
        setError(saveError.message || "ذخیره اطلاعات در حساب شما ناموفق بود.");
        toast.error("اطلاعات ذخیره نشد", { description: "پیش از ادامه دوباره تلاش کنید." });
        return;
      }
    }
    ...
    <CheckoutLayout currentStep="shipping" items={items} subtotal={subtotal} discount={discount} couponCode={couponCode} shippingMethod={form}>
      ...
      <Card sx={{ width: "100%", minWidth: 0, bgcolor: "#ffffff", color: "var(--color-text-primary)", border: "1px solid var(--color-border)" }}>
        <CardContent sx={{ p: { xs: 3, md: 4 }, minHeight: { xs: 680, md: 720 }, boxSizing: "border-box" }}>
          <Typography variant="h5" sx={{ fontWeight: 900, mb: 1 }}>نشانی ارسال</Typography>
          <Typography sx={{ color: "var(--color-text-secondary)", mb: 3 }}>محل تحویل سفارش Weluxo را انتخاب کنید.</Typography>
          <Box component="form" onSubmit={handleSubmit}>
            <Stack spacing={2.5}>
              <TextField select label="کشور" value={form.country} onChange={changeCountry} required fullWidth sx={fieldSx} SelectProps={{ MenuProps: menuProps }}>
                <MenuItem value="" disabled>یک کشور انتخاب کنید</MenuItem>
                {countryOptions.map((country) => <MenuItem key={country.code} value={country.code}>{country.label}</MenuItem>)}
              </TextField>
              <TextField label="نام و نام خانوادگی" value={form.fullName} onChange={update("fullName")} required fullWidth autoComplete="shipping name" sx={fieldSx} />
              <TextField label="نشانی خیابان" value={form.addressLine1} onChange={update("addressLine1")} required fullWidth autoComplete="shipping street-address" sx={fieldSx} />
              <TextField label="واحد، آپارتمان و… (اختیاری)" value={form.addressLine2} onChange={update("addressLine2")} fullWidth autoComplete="shipping address-line2" sx={fieldSx} />
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "minmax(0, 1fr)", sm: "repeat(2, minmax(0, 1fr))" }, gap: 2, width: "100%" }}>
                <Box sx={{ minWidth: 0, width: "100%" }}><TextField label="شهر" value={form.city} onChange={update("city")} required fullWidth autoComplete="shipping address-level2" sx={fieldSx} /></Box>
                <Box sx={{ minWidth: 0, width: "100%" }}>
                  {stateOptions.length ? (
                    <TextField select label="استان / منطقه" value={form.region} onChange={update("region")} required fullWidth disabled={!form.country} sx={{ ...fieldSx, width: "100%" }} SelectProps={{ MenuProps: menuProps }}>
                      <MenuItem value="" disabled>یک منطقه انتخاب کنید</MenuItem>
                      {stateOptions.map((region) => <MenuItem key={region.code} value={region.code}>{region.label}</MenuItem>)}
                    </TextField>
                  ) : (
                    <TextField label="استان / منطقه" value={form.region} onChange={update("region")} required fullWidth disabled={!form.country} sx={{ ...fieldSx, width: "100%" }} />
                  )}
                </Box>
                <Box sx={{ minWidth: 0, width: "100%" }}><TextField label="کد پستی" value={form.postalCode} onChange={update("postalCode")} required fullWidth autoComplete="shipping postal-code" sx={fieldSx} /></Box>
              </Box>

              <Box sx={{ pt: 1 }}>
                <FormControl fullWidth>
                  <FormLabel sx={{ color: "var(--color-text-primary)", mb: 1 }}>روش ارسال</FormLabel>
                  <Button type="button" variant="outlined" onClick={handleLoadShippingOptions} disabled={shippingLoading} sx={{ alignSelf: "flex-start", mb: 1.5, borderRadius: 999 }} startIcon={shippingLoading ? <CircularProgress size={16} color="inherit" /> : undefined}>
                    {shippingLoading ? "در حال بارگذاری…" : "نمایش روش‌های ارسال"}
                  </Button>
                  <RadioGroup value={form.logisticName || form.method || ""} onChange={selectShippingOption}>
                    {shippingOptions.map((option) => (
                      <FormControlLabel
                        ...
                        control={<Radio />}
                        label={<Box><Typography>{option.label || option.logisticName}</Typography><Typography variant="body2" color="var(--color-text-secondary)">{option.window || "زمان تحویل نمایش داده می‌شود"} · {formatMoney(Number(option.cost || 0))}</Typography></Box>}
                      />
                    ))}
                  </RadioGroup>
                  {!shippingOptions.length && <Typography variant="body2" color="var(--color-text-secondary)">نشانی خود را وارد کنید و روش‌های ارسال موجود را ببینید.</Typography>}
                </FormControl>
              </Box>
              <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
                <Button type="submit" variant="contained" size="large" disabled={shippingLoading} sx={{ borderRadius: 999, py: 1.3, fontWeight: 800 }}>ادامه به پرداخت</Button>
                <Button component={Link} href="/checkout/information" variant="outlined" sx={{ color: "var(--color-primary)", borderColor: "var(--color-primary)", borderRadius: 999 }}>بازگشت به اطلاعات</Button>
              </Stack>
            </Stack>
          </Box>
        </CardContent>
