...
import defaultSiteChrome from "../../data/site-chrome.json";
...
export default async function RootLayout({ children }) {
  const [siteSettings, siteChrome] = await Promise.all([
    getSiteSettingsServer(),
    readPageContent("site-chrome.json", defaultSiteChrome),
  ]);
      ...
      <body style={{ ...buildSiteColorVars(siteSettings), ...buildSiteFontVars(siteSettings) }}>
        <AppRouterCacheProvider>
          <SiteThemeProvider siteSettings={siteSettings}>
            ...
            <ConditionalShell initialChrome={siteChrome}>{children}</ConditionalShell>
            <ToastProvider />
          </SiteThemeProvider>
        </AppRouterCacheProvider>
      </body>
